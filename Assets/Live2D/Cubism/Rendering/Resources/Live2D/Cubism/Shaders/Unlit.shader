...
Shader "Live2D Cubism/Unlit"
{
    Properties
    {
		...
		[PerRendererData] _MinZTex("MinZ Texture", 2D) = "white" {}
		[PerRendererData] _MaxZTex("MaxZ Texture", 2D) = "white" {}
        [PerRendererData] cubism_ModelOpacity("Model Opacity", Float) = 1
		[PerRendererData] cubism_ZbuffOpacity("Zbuff Opacity", Float) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata IN)
            {
                ...
                float zPos = tex2Dlod(_MinZTex, float4(IN.texcoord.xy,0,0)).r * cubism_ZbuffOpacity + tex2Dlod(_MaxZTex, float4(IN.texcoord.xy,0,0)).r * (1 - cubism_ZbuffOpacity);
                float4 newPos = float4(IN.vertex.xy, -zPos * 0.4 ,IN.vertex.w);

				OUT.vertex	 = UnityObjectToClipPos(newPos);
